...
```
<img width="1909" height="714" alt="image" src="https://github.com/user-attachments/assets/7c951238-c03b-4619-930e-5a96db8db74d" />
...
คำตอบ:
```
Image คือไฟล์ต้นแบบ (template) สำหรับสร้าง Container
Container คือ Image ที่ถูกรันแล้ว
ตัวอย่าง: Image moodle:latest  รันเป็น Container Moodle
```
...
คำตอบ:
```
 2 ตัว moodle_app : ให้บริการเว็บ Moodle
db : จัดเก็บฐานข้อมูล (MySQL/MariaDB)
```
...
```
ใช้ Named Volume
ข้อดี: ข้อมูลไม่หายเมื่อปิด/ลบ Container
ข้อเสีย: ต้องจัดการลบเองเมื่อไม่ใช้
```
...
คำตอบ:
```
เชื่อมต่อ Container ให้อยู่ในเครือข่ายเดียวกัน Container ติดต่อกันผ่าน ชื่อ service เช่น db
...
คำตอบ:
```
กำหนดลำดับการเริ่ม Container ให้ db เริ่มก่อน moodle_app
```
...
คำตอบ:
```
db คือชื่อ Container ฐานข้อมูล ใช้ localhost ไม่ได้ เพราะแต่ละ Container แยกกัน
```
...
คำตอบ:
```
Docker: ติดตั้งเร็ว ย้ายง่าย สภาพแวดล้อมเหมือนกัน
ปกติ: ควบคุมระบบได้ละเอียด แต่ติดตั้งยากกว่า
```
...
คำตอบ:
```
วิธีตรวจสอบ:เช็ก log ด้วย docker-compose logs ตรวจสอบชื่อ DB, user, password
วิธีแก้ไข:แก้ค่า Environment Variables ให้ตรง Restart container
...
คำตอบ:
```
Container หยุดและถูกลบ Volume ถูกลบทั้งหมด ข้อมูลฐานข้อมูล หายถาวร
